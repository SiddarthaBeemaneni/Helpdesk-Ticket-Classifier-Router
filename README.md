...
1. Push this repo to GitHub (or GitLab/Bitbucket) and create a new **Web
   Service** on [Render](https://render.com), pointing it at the repo.
2. Set **Root Directory** to `backend`.
3. Build command: `pip install -r requirements.txt`
4. Start command (Render reads this from the `Procfile` automatically):
   `gunicorn app:app --bind 0.0.0.0:$PORT`
5. Add an environment variable `SECRET_KEY` with a random value (used to
   sign Flask sessions — without it a new one is generated on every
   restart, which logs everyone out).
6. Deploy. Render assigns the public URL and injects `PORT` automatically.

**Note on the database:** Render's free-tier disk is ephemeral — the
SQLite file (`ticketsense.db`) will be wiped on every redeploy or if the
service spins down and back up. That's fine for a demo, but for anything
persistent you'd want a [Render Disk](https://render.com/docs/disks) (paid)
or to switch to a managed Postgres database.
